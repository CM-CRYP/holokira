...
test('database migration and reservation workflows in isolated PostgreSQL', async (t) => {
  ...
  await t.test('submission limits protect direct inserts and cannot be changed by visitors', async () => {
    await role('anon')
    const sell = (id, email = 'seller@example.com') => db.query("insert into public.sell_requests(id,full_name,email,card_list) values ($1,'Test Seller',$2,'Pikachu')", [id, email])
    for (let i = 0; i < 5; i++) await sell(`SELL-test-${i}`)
    await assert.rejects(sell('SELL-blocked', ' SELLER@example.com '), /Trop de demandes/)
    await assert.rejects(read('select * from private.submission_limits'))
    await role('authenticated', otherId)
    await assert.rejects(sell('SELL-non-admin'), /Trop de demandes/)
    await role('authenticated', adminId)
    await sell('SELL-admin')
    await owner()
    assert.deepEqual(await read("select id from public.sell_requests where id='SELL-blocked'"), [])
  })
  await t.test('RPC upserts are limited and expired hourly counters reset', async () => {
    await role('anon')
    const alert = () => db.query("select public.create_stock_alert('multi','alerts@example.com')")
    for (let i = 0; i < 20; i++) await alert()
    await assert.rejects(alert(), /Trop de demandes/)
    await owner()
    await db.exec("update private.submission_limits set window_start=now()-interval '2 hours' where scope='stock_alerts' and period='hour'")
    await role('anon'); await alert()
    await owner()
    assert.equal((await read("select count(*)::int as n from public.stock_alerts where customer_email='alerts@example.com'"))[0].n, 1)
  })
  await t.test('global and daily ceilings hold and blocked reservations preserve inventory', async () => {
    await owner()
    await db.exec("update private.submission_limits set submissions=200 where scope='reservations' and subject='global' and period='hour'")
    await role('anon')
    await assert.rejects(reserve('RESA-rate-blocked', [['atomic-a', 1]], { customer_email: 'new-address@example.com' }), /Trop de demandes/)
    assert.equal((await card('atomic-a')).stock, 1)
    await owner()
    assert.deepEqual(await read("select id from public.reservations where id='RESA-rate-blocked'"), [])
    await db.exec("update private.submission_limits set submissions=15 where scope='japan_requests' and subject='email:'||md5('test@example.com') and period='day'")
    await role('anon')
    await assert.rejects(db.query('select public.create_japan_request($1)', [JSON.stringify({ customer_name: 'Test Client', customer_email: 'test@example.com', card_list: 'Pikachu promo', budget: 100 })]), /Trop de demandes/)
  })
})
